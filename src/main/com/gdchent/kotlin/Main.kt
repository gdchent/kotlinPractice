...
//
   ...
   var componentX:ComponentX=ComponentX()
    ...
    println("$a == $b")
    //lambda表达式经典例子
    val kotlinFunction=KotlinFunction("james",23,{name,age->
            println(name + ",age:" + age)
    })
    kotlinFunction.setLambda("james",23,{name,age->
        println(name + ",age:" + age)
    })
}
...
fun testObject() {
    var javaPerson: KotlinPerson = KotlinPerson("java", "java_sound", 666)  //kotlin里面调用Java实例
